...
# ⚠️ غيّر الـ IP لو تغيّر (EC2 Public IP)
DEFAULT_API_URL      = _env("IDS_API_URL",      "http://44.203.215.0:30500/api/Traffic/ProcessTraffic")
DEFAULT_INTERFACE    = _env("IDS_INTERFACE",     r"\Device\NPF_{7389ADBB-9C23-447C-A4DF-3C7EAF6709B0}")
DEFAULT_DEVICE_ID    = _env("IDS_DEVICE_ID",     "DESKTOP-GSHOI10")
DEFAULT_BATCH_SIZE   = int(_env("IDS_BATCH_SIZE",    "20"))
DEFAULT_BATCH_INTERVAL = float(_env("IDS_BATCH_INTERVAL", "2.0"))
...
def build_flow_dict(flow, device_id: str) -> dict:
    data = {field: getattr(flow, field, None) for field in FLOW_FIELDS}
    data["device_id"] = device_id
    return data


class FlowSender:
    def __init__(self, api_url, session, batch_size, batch_interval, timeout):
        self.api_url = api_url
        self.session = session
        self.batch_size = batch_size
        self.batch_interval = batch_interval
        self.timeout = timeout
        self.queue = queue.Queue(maxsize=DEFAULT_QUEUE_MAX_SIZE)
    ...
    def submit(self, flow_data: dict):
        try:
            ...
            try:
                ...
                logger.warning("Queue full, dropping flow")
    ...
    def _send_batch(self, batch: list):
        try:
            ...
            for flow_data, result in zip(batch, results):
                tag = "⚠️  ANOMALY" if result.get("is_anomaly") else "✅ benign"
                logger.info(
                    "[%s -> %s] %s | class=%s conf=%.1f%% anomaly=%.1f%%",
                    flow_data.get("src_ip"), flow_data.get("dst_ip"), tag,
                    result.get("predicted_class"),
                    (result.get("class_confidence") or 0) * 100,
                    (result.get("anomaly_probability") or 0) * 100,
                )
...
def main() -> None:
    ...
    logger.info("Windows Agent started on interface: %s", args.interface)
    logger.info("Forwarding to: %s", args.api_url)
    ...
    try:
        for flow in streamer:
            sender.submit(build_flow_dict(flow, args.device_id))
